...
IntList getIndexes(String Charers, String Char) {
  IntList indexes = new IntList();
  for (int i = 0; i < Charers.length(); i ++) {
    String currChar = Charers.substring(i, i+1);
    if (currChar.contains(Char)) {
      indexes.append(i);
    }
  }
  return indexes;
